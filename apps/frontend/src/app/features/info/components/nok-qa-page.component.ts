...
export class NokQaPageComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  popularQuestions: QaItem[] = [
    ...
    },
    {
      ...
      answer: 'Свидетельство о квалификации действует 5 лет с момента выдачи. По истечении срока необходимо повторно пройти процедуру НОК. Информация о выданных свидетельствах хранится в Федеральном реестре сведений о документах об образовании и квалификации (ФРДО). Рекомендуем начать подготовку к пересдаче за 3-4 месяца до окончания срока действия.'
    },
    {
      question: 'Можно ли пересдать экзамен НОК?',
      answer: 'Да, при несдаче экзамена вы имеете право на повторную попытку. Ограничений по количеству пересдач нет, однако каждая попытка оплачивается отдельно. Пересдать можно через 30 дней после неудачной попытки. Мы предоставляем гарантию: при несдаче после полной подготовки по нашей программе --- бесплатная повторная подготовка и сопровождение на пересдаче.'
    },
    {
    ...
    },
    {
      ...
      answer: 'Весь процесс занимает от 2 до 4 недель. Подача документов и их проверка --- 3-5 рабочих дней. Назначение даты экзамена --- в течение 10 дней. Сам экзамен занимает от 2 до 6 часов в зависимости от квалификации. Результаты объявляются в течение 5 рабочих дней. Свидетельство выдаётся в течение 10 рабочих дней после успешной сдачи.'
    }
  ];
  ...
  categories: QaCategory[] = [
    {
      ...
      items: [
        ...
        },
        {
          ...
          answer: 'Да, трудовая книжка или её заверенная копия необходима для подтверждения стажа работы по специальности. Минимальный стаж зависит от уровня квалификации: для 5-го уровня --- от 3 лет, для 6-го --- от 5 лет, для 7-го --- от 7 лет. Также подойдут трудовые договоры или справки от работодателей.'
        },
        {
      ...
      items: [
        {
          ...
          answer: 'Экзамен включает теоретическую часть (компьютерное тестирование --- 40-60 вопросов за 60-90 минут) и практическую часть (решение ситуационных задач или защита портфолио). Формат зависит от конкретной квалификации. Для строительных специальностей обычно предусмотрены обе части.'
        },
        {
        ...
        },
        {
          ...
          answer: 'Для теоретической части необходимо правильно ответить минимум на 70% вопросов. Практическая часть оценивается экспертной комиссией по установленным критериям. Для получения свидетельства необходимо успешно пройти обе части экзамена.'
        },
        {
      ...
      items: [
        {
          ...
          answer: 'Результаты теоретической части доступны сразу после завершения тестирования. Итоговое решение экспертной комиссии по обеим частям экзамена формируется в течение 5 рабочих дней и направляется соискателю по электронной почте.'
        },
        {
          question: 'Как получить свидетельство о квалификации?',
          answer: 'Свидетельство выдаётся в течение 10 рабочих дней после положительного решения экспертной комиссии. Получить можно лично в ЦОК или по почте. Данные о свидетельстве вносятся в Федеральный реестр, что позволяет работодателям проверить его подлинность онлайн.'
        },
        {
          question: 'Что делать, если результаты экзамена кажутся несправедливыми?',
          answer: 'Вы можете подать апелляцию в ЦОК в течение 30 дней с момента получения результатов. Апелляция рассматривается в течение 10 рабочих дней. Если решение не изменится, можно обратиться в Совет по профессиональным квалификациям (СПК).'
        },
        {
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          answer: 'Да, согласно статье 219 Налогового кодекса РФ, расходы на прохождение независимой оценки квалификации можно вернуть через социальный налоговый вычет. Максимальная сумма вычета --- 120 000 рублей в год (возврат до 15 600 рублей). Для оформления потребуются договор с ЦОК и платёжные документы.'
        },
        {
  ...
  keyFacts = [
    ...
    { number: '5 лет', label: 'Срок действия', description: 'Период действия свидетельства о квалификации' },
    { number: '70%', label: 'Проходной балл', description: 'Минимальный порог правильных ответов на теоретическом экзамене' },
