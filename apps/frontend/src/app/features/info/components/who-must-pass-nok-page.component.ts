...
export class WhoMustPassNokPageComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  lawRequirements = [
    {
      ...
      description: 'Закон от 03.07.2016 устанавливает правовую основу системы независимой оценки квалификации. НОК проводится по инициативе соискателя или по направлению работодателя. Обязательность определяется отраслевыми нормативными актами.',
      color: 'blue'
    },
    {
      icon: 'landmark',
      title: 'Градостроительный кодекс РФ',
      description: 'Статья 55.5-1 ГрК РФ определяет требования к специалистам по организации строительства, проектирования и изысканий. Для включения в Национальный реестр специалистов (НРС) необходимо действующее свидетельство НОК.',
      color: 'emerald'
    },
    {
      icon: 'file-check',
  ...
  faqItems = [
    ...
    },
    {
      ...
      answer: 'Свидетельство о квалификации действует от 3 до 5 лет в зависимости от квалификации. С 27 февраля 2025 года для ГИП (7 уровень) и специалистов по строительству ОТС (6 уровень) срок составляет 3 года, для остальных --- 5 лет. Рекомендуется начинать подготовку за 2--3 месяца до окончания срока действия свидетельства.'
    },
    {
    ...
    },
    {
      ...
      answer: 'С 1 марта 2025 года НОК стала обязательной для специалистов, ответственных за пожарную безопасность на опасных производственных объектах. Для остальных организаций НОК по ОПБ остаётся добровольной, однако всё больше заказчиков и подрядчиков требуют наличие свидетельства НОК при допуске к объектам.'
    }
  ];
