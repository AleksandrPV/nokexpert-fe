...
export class NokProcedurePageComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  procedureSteps = [
    ...
    },
    {
      ...
      description: 'Компьютерное тестирование по вопросам профессионального стандарта. 50 вопросов за 60 минут (для НОСТРОЙ). Необходимо набрать не менее 72% правильных ответов (36 из 50) для успешного прохождения.',
      duration: '60 минут',
      color: 'blue'
    },
    {
  ...
  examFormatItems = [
    ...
    },
    {
      ...
      description: 'Для успешного прохождения необходимо правильно ответить минимум на 72% вопросов теоретической части (36 из 50 для НОСТРОЙ) и получить положительную оценку по практической.',
      detail: 'от 72%'
    },
    {
      icon: 'users',
  ...
  afterExamItems = [
    ...
    {
      icon: 'calendar',
      title: 'Срок действия --- 3–5 лет',
      description: 'Свидетельство о квалификации действует от 3 до 5 лет с даты выдачи в зависимости от квалификации. С 27.02.2025 для ГИП (7 ур.) и спец. по строительству ОТС (6 ур.) --- 3 года, для остальных --- 5 лет.'
    }
  ];
  ...
  faqItems = [
    ...
    },
    {
      ...
      answer: 'В рамках одной процедуры: пересдача теоретической части допускается 1 раз, пересдача практической части не предусмотрена. Однако соискатель может подать новое заявление на полный экзамен --- количество таких попыток не ограничено, каждая оплачивается отдельно.'
    },
    {
      question: 'Сколько стоит прохождение НОК?',
      answer: 'Стоимость зависит от квалификации и ЦОК. В 2025 году --- от 19 000 до 25 000 рублей за один профессиональный экзамен. В стоимость входит теоретическая и практическая части, а также оформление свидетельства при успешной сдаче. С ноября 2025 года возможно повышение тарифов.'
    },
    {
      question: 'Что делать, если не согласен с результатом экзамена?',
      answer: 'Соискатель имеет право подать апелляцию в ЦОК в течение 30 календарных дней после получения результата. Апелляция регистрируется в течение 7 дней и рассматривается в течение 60 дней. Решение доводится до заявителя в течение 7 дней.'
    },
    {
      question: 'Нужно ли проходить НОК повторно при смене работы?',
      answer: 'Нет, свидетельство о квалификации привязано к специалисту, а не к работодателю. Оно действует от 3 до 5 лет (в зависимости от квалификации) независимо от смены места работы и признаётся всеми организациями на территории России.'
    },
    {
