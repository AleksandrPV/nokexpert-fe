...
export class WhatIsNokPageComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  keyFacts = [
    ...
    { number: '500+', label: 'Центров ЦОК', description: 'Аккредитованных центров оценки квалификаций в России' },
  ...
  faqItems = [
    ...
    },
    {
      ...
      answer: 'Свидетельство о квалификации действует 5 лет с момента выдачи. По истечении срока необходимо повторно пройти процедуру независимой оценки квалификации. Данные о свидетельстве хранятся в Федеральном реестре сведений о квалификациях.'
    },
    {
    ...
    },
    {
      ...
      answer: 'При несдаче экзамена соискатель имеет право на повторную попытку. Ограничений по количеству попыток нет, но каждая попытка оплачивается отдельно. Мы предоставляем гарантию: при несдаче после полной подготовки по нашей программе --- бесплатная повторная подготовка.'
    }
  ];
