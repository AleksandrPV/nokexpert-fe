...
export class SeoService {
  ...
  private readonly defaultImage = '/assets/images/og-default.jpg'; // Must be JPG/PNG 1200x630px, NOT SVG
  ...
  private readonly defaultSeoData: SeoData = {
    title: 'Сдать НОК с гарантией — НОК Эксперт | nok-expert.ru',
    description: 'Подготовка и сдача НОК НОСТРОЙ, НОПРИЗ, ОПБ и ЖКХ с гарантией результата. 99% успешных сдач, 1500+ специалистов в НРС. Бесплатная консультация и проверка документов.',
    keywords: 'НОК, независимая оценка квалификаций, сдать НОК, НОК НОСТРОЙ, НОК НОПРИЗ, НОК ОПБ, НОК ЖКХ, подготовка к НОК, НРС',
  ...
  setHomePageSeo(): void {
    this.setSeoData({
      title: 'Сдать НОК с гарантией — НОК Эксперт | nok-expert.ru',
      description: 'Подготовка и сдача НОК НОСТРОЙ, НОПРИЗ, ОПБ и ЖКХ с гарантией результата. 99% успешных сдач, 1500+ специалистов в НРС. Бесплатная консультация и проверка документов.',
      keywords: 'НОК, независимая оценка квалификаций, сдать НОК, НОК НОСТРОЙ, НОК НОПРИЗ, НОК ОПБ, НОК ЖКХ, подготовка к НОК, НРС, центр оценки квалификации',
  ...
  /**
   * Комбинированные Structured Data для главной страницы
   * Включает Organization, WebSite, LocalBusiness и BreadcrumbList в @graph
   */
    ...
    return {
      ...
      "@graph": [
        this.getOrganizationStructuredData(),
        {
          ...
          "url": this.baseUrl,
          "name": "НОК Эксперт",
          "description": "Подготовка и сдача независимой оценки квалификации (НОК) для специалистов НОСТРОЙ, НОПРИЗ, ОПБ и ЖКХ",
          "publisher": {
        ...
        },
        {
          ...
          "url": this.baseUrl,
          "name": "Сдать НОК с гарантией — НОК Эксперт",
          "description": "Подготовка и сдача НОК НОСТРОЙ, НОПРИЗ, ОПБ и ЖКХ с гарантией результата.",
          "isPartOf": {
            "@id": `${this.baseUrl}/#website`
          },
          "about": {
        ...
        this.getLocalBusinessStructuredData(),
        {
          "@type": "BreadcrumbList",
          "itemListElement": [
            {
  ...
  setNokNostroyPageSeo(): void {
    this.setSeoData({
      title: 'НОК НОСТРОЙ — подготовка и сдача | НОК Эксперт',
      description: 'Подготовка к НОК НОСТРОЙ от 35 000 руб. 600+ вопросов, пробный экзамен, сопровождение до получения свидетельства. 98% сдают с первого раза.',
      ...
      ogTitle: 'НОК НОСТРОЙ — подготовка и сдача | НОК Эксперт',
      ogDescription: 'Подготовка к НОК НОСТРОЙ от 35 000 руб. 600+ вопросов, пробный экзамен, сопровождение до получения свидетельства.',
      ogImage: '/assets/images/nok-nostroy-og.jpg',
      ogUrl: `${this.baseUrl}/services/nok-nostroy`,
      twitterTitle: 'НОК НОСТРОЙ — подготовка и сдача',
      twitterDescription: 'Подготовка к НОК НОСТРОЙ от 35 000 руб. 98% сдают с первого раза. Полное сопровождение.',
  ...
  setNokNoprizPageSeo(): void {
    this.setSeoData({
      title: 'НОК НОПРИЗ — подготовка и сдача экзамена | НОК Эксперт',
      description: 'Подготовка к НОК НОПРИЗ от 40 000 руб. 800+ вопросов, помощь с портфолио, гарантия сдачи с первого раза. Для проектировщиков и изыскателей.',
  ...
  setNokNostroyInfoPageSeo(): void {
    this.setSeoData({
      title: 'НОК НОСТРОЙ — экзамен для строителей | НОК Эксперт',
      description: 'Полная информация о НОК НОСТРОЙ: профессиональные стандарты, уровни квалификации 5/6/7, формат экзамена, проходной балл. Всё о строительной квалификации и Национальном объединении строителей.',
      keywords: 'НОК НОСТРОЙ, НОСТРОЙ, экзамен строителей, квалификация строителя, НРС, профессиональные стандарты строительство, 16.025, 16.032, независимая оценка квалификации',
  ...
  setNokOpbPageSeo(): void {
    this.setSeoData({
      title: 'НОК МЧС --- для ответственных по пожарной безопасности | НОК Эксперт',
      description: 'НОК МЧС от 13 900 руб. Подготовка и сдача экзамена по пожарной безопасности и аварийно-спасательным работам. Полное сопровождение, гарантия результата.',
      keywords: 'НОК МЧС, пожарная безопасность, ответственный по пожарной безопасности, НОК ОПБ, аварийно-спасательные работы, независимая оценка квалификации, СПК ЧС, НОК пожарная безопасность',
      canonical: `${this.baseUrl}/services/nok-opb`,
      ogTitle: 'НОК МЧС --- для ответственных по пожарной безопасности',
      ogDescription: 'Подготовка и сдача НОК по пожарной безопасности и МЧС от 13 900 руб. Полное сопровождение до получения свидетельства.',
      ogImage: '/assets/images/nok-opb-og.jpg',
      ogUrl: `${this.baseUrl}/services/nok-opb`,
      twitterTitle: 'НОК МЧС --- пожарная безопасность',
      twitterDescription: 'Подготовка и сдача НОК по пожарной безопасности и МЧС от 13 900 руб. Полное сопровождение до получения свидетельства.',
  ...
  setExamPreparationPageSeo(): void {
    this.setSeoData({
      title: 'Услуги по подготовке к НОК - Профессиональное сопровождение | НОК Эксперт',
      description: 'Профессиональные услуги по подготовке к НОК. Базовый, премиум и экспресс пакеты. Сопровождение во всех регионах России. Гарантия успешной сдачи.',
      keywords: 'услуги НОК, подготовка к НОК, сопровождение НОК, пакеты НОК, подготовка по городам, центры НОК',
      canonical: `${this.baseUrl}/info/exam-preparation`,
      ogTitle: 'Услуги по подготовке к НОК - Профессиональное сопровождение',
      ogDescription: 'Профессиональные услуги по подготовке к НОК. Базовый, премиум и экспресс пакеты. Сопровождение во всех регионах России.',
      ...
      structuredData: this.getInfoPageStructuredData({
        ...
        tags: ['услуги НОК', 'подготовка к экзамену', 'сопровождение', 'гарантия']
      })
    });
  }
  ...
  private updateCanonicalUrl(url: string): void {
    // Canonical URL должен быть в SSR-рендере для поисковых ботов
    try {
      ...
      console.warn('updateCanonicalUrl: DOM manipulation failed', e);
    }
  }
  ...
  private updateStructuredData(data: any): void {
    // DOCUMENT token работает и на сервере (SSR) и в браузере
    // Структурированные данные должны быть в SSR-рендере для поисковых ботов
    try {
      ...
      // Fallback: если DOM-манипуляция не поддерживается на сервере
      console.warn('updateStructuredData: DOM manipulation failed', e);
    }
  }
  ...
  private getOrganizationStructuredData(): any {
    return {
      ...
      "logo": {
        ...
        "contentUrl": `${this.baseUrl}/assets/images/logo.png`,
        "caption": "НОК Эксперт — подготовка к независимой оценке квалификации"
      },
      "image": `${this.baseUrl}/assets/images/og-default.jpg`,
      "description": "Подготовка и сдача независимой оценки квалификации (НОК) для специалистов НОСТРОЙ, НОПРИЗ, ОПБ и ЖКХ с гарантией результата",
      "address": {
        "@type": "PostalAddress",
  ...
  setNokEmergencyPageSeo(): void {
    this.setSeoData({
      title: 'НОК МЧС --- аварийно-спасательные работы | НОК Эксперт',
      description: 'Независимая оценка квалификации для специалистов аварийно-спасательных и других неотложных работ. Подготовка к НОК МЧС от 32 000 руб. Гарантия сдачи с первого раза.',
      keywords: 'НОК МЧС, аварийно-спасательные работы, НОК спасатель, ГОЧС, чрезвычайные ситуации, НОК ЧС, независимая оценка квалификации МЧС',
      canonical: `${this.baseUrl}/services/nok-emergency`,
      ogTitle: 'НОК МЧС --- аварийно-спасательные работы | НОК Эксперт',
      ogDescription: 'Подготовка и сдача НОК для специалистов аварийно-спасательных и других неотложных работ. Полное сопровождение до получения свидетельства.',
      ogImage: '/assets/images/nok-emergency-og.jpg',
      ogUrl: `${this.baseUrl}/services/nok-emergency`,
      twitterTitle: 'НОК МЧС --- аварийно-спасательные работы',
      twitterDescription: 'Независимая оценка квалификации для специалистов МЧС. Подготовка от 32 000 руб.',
